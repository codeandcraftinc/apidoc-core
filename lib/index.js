        ...
        if (parsedFiles.length > 0) {
            ...
            blocks.sort(function(a, b) {
                ...
                if (nameA === nameB) {
                    ...
                    else if (/\d{4}-\d{2}-\d{2}/.test(a.version) && /\d{4}-\d{2}-\d{2}/.test(b.version))
                        return (Date.parse(a.version) > Date.parse(b.version)) ? -1 : 1;
                    else
                        throw new Error('Error parsing API versions');
                }
